...
import { siShopify, siSupabase } from "simple-icons";
import { cn } from "@/lib/utils";

export type Integration = "shopify" | "supabase" | "lark";

const integrationOrder: Integration[] = ["shopify", "supabase", "lark"];
...
} satisfies Record<Integration, { label: string; color: string; path?: string }>;
...
export function detectIntegrations(...values: ReactNode[]): Integration[] {
  const text = values.map(textFromNode).join(" ");

  return integrationOrder.filter((integration) => {
    if (integration === "shopify") return /\b(shopify|myshopify)\b/i.test(text);
    if (integration === "supabase") return /\b(supabase|postgrest)\b/i.test(text);
    return /\b(lark|larksuite|feishu|bitable)\b/i.test(text);
  });
}
...
export function IntegrationLogo({
  ...
  return (
    <span
      className={cn(
        "inline-grid h-6 w-6 shrink-0 place-items-center rounded-md border bg-background shadow-sm",
        className,
      )}
    ...
    >
      <svg viewBox={integration === "lark" ? "62.16 94.5 407.87 324.19" : "0 0 24 24"} className="h-3.5 w-3.5">
        {integration === "lark" ? (
          larkPaths.map((path) => <path key={path.fill} fill={path.fill} d={path.d} />)
        ) : (
          <path fill="currentColor" d={integration === "shopify" ? siShopify.path : siSupabase.path} />
        )}
      </svg>
    </span>
  );
...
export function IntegrationBadge({ integration }: { integration: Integration }) {
  ...
  return (
    <span className="inline-flex h-7 items-center gap-1.5 rounded-full border bg-background px-2.5 text-xs font-medium text-foreground/80 shadow-sm">
      <IntegrationLogo integration={integration} className="h-[18px] w-[18px] border-0 bg-transparent shadow-none" />
      {meta.label}
    </span>
  );
}

export function IntegrationBadges({ integrations }: { integrations: Integration[] }) {
  if (integrations.length === 0) return null;

  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {integrations.map((integration) => (
        <IntegrationBadge key={integration} integration={integration} />
      ))}
    </div>
  );
}
