...
export const gotchas: GotchaRow[] = [
  ...
  { n: 2, title: "Colima Docker VM không tự dậy sạch sau reboot", body: "Tính tới 2026-05-27, lệnh `colima` báo 'lima not found, run brew install lima'. Triệu chứng: reboot xong → supabase.patiagency.com 502, mọi sync rớt. Fix tay: `brew install lima && colima start`. Fix lâu dài: plist launchd để `colima start` khi boot.", sev: "danger" },
  { n: 3, title: "NAT của ISP nhà giết tunnel Cloudflare âm thầm", body: "Triệu chứng: card $0, section trống, 502 thỉnh thoảng. Nguyên nhân thật: ISP ở nhà drop connection idle của cloudflared edge. Fix bằng `protocol: http2`, `edge-ip-version: 4`, `tcpKeepAlive: 30s`, `retries: 10` trong ~/.cloudflared/config.yml. ĐỪNG 'dọn dẹp' config về default.", sev: "danger" },
  ...
  { n: 15, title: "postgres-meta /pg/query KHÔNG XÁC THỰC", body: "https://supabase.patiagency.com/pg/query chạy SQL bất kỳ với quyền supabase_admin mà KHÔNG auth. Vừa là cửa thoát hữu ích, vừa là lỗ hổng bảo mật.", sev: "danger" },
